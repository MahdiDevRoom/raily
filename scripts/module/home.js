...
export default {
    ...
    _matchDate(task, filter) {
        ...
        if (filter === 'today') return this._sameDay(taskDate, today);
        if (filter === 'past') return taskDate < today && !this._sameDay(taskDate, today);
        if (filter === 'future') return taskDate > today && !this._sameDay(taskDate, today);
    ...
    _createTask(task) {
        ...
        const translatedCategories = categories
            .filter(Boolean)
            .map(cat => this.categoryLabels[cat] || cat)
            .join('، ');
            ...
            <div class="content">
                <div class="title">
                    ...
                    <div class="meta">
                        ...
                        <span>${this._escape(translatedCategories)}</span>
                    </div>
                </div>
                ${task.note ? `<div class="note">${this._escape(task.note)}</div>` : ''}
            </div>
            <div class="actions">
                <button class="check symbol" type="button" title="${task.completed ? 'بازگردانی' : 'انجام شد'}">
                    ${task.completed ? 'undo' : 'check'}
                </button>
                <button class="delete symbol" type="button" title="حذف">
                    delete
                </button>
            </div>
        `;
    ...
    async _confirmDelete(task) {
        if (typeof CONFIRM?.open !== 'function') {
            return window.confirm(`آیا از حذف «${task.title}» مطمئن هستید؟`);
        }
    ...
    async _saveTasks(tasks) {
        await this.tasksStore.setItem('tasks', tasks);
    },
    ...
    _getToday() {
        ...
        return this._gregorianToJalali(date.getFullYear(), date.getMonth() + 1, date.getDate());
    },
